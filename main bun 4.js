...
console.log(vnode);
  // Handle v-if before creating element
  if (vnode.props && vnode.props["v-if"] !== undefined) {
    let show = false;
    try {
      show = new Function(...Object.keys(ctx), `return ${vnode.props["v-if"]}`)(
        ...Object.values(ctx)
      );
    } catch {
      show = false;
    ...
    if (!show) {
      return document.createComment("v-if false"); // placeholder node
    }
  }

  const el = document.createElement(vnode.tag);
  for (const key in vnode.props) {
    if (key === "v-if") continue; // skip v-if as it's handled
