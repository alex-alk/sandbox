  ...
  set(target, variableName, value) {
    ...
    if (Object.keys(bindings).length !== 0) {
        if (bindings[variableName]) {
            ...
            if (bindings[variableName]['v-bind:class']) {
                for (const el of bindings[variableName]['v-bind:class']) {
...
export function init(component) {
    const all = component.querySelectorAll('[v-text], [v-bind\\:src], [v-if], [v-for], [v-on\\:click], [v-on\\:mouseover], [v-bind\\:class], [v-bind\\:disabled]');
    ...
    for (const el of all) {
        ...
        for (const { attr, key } of directives) {
            ...
            if (attr === 'v-bind:class') {
                if (value) {
                    el.classList.add(value);
                }
            }

            if (attr === 'v-bind:disabled') {
                el.disabled = value;
            }
