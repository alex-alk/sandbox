...
// 2) Render into DOM, tracking dynamic text, attributes, and conditions (v-if/v-else)
export function render(vnode, ctx, dynamicNodes = [], dynamicAttrs = [], dynamicConds = []) {
  ...
  if ('v-if' in vnode.props || 'v-else' in vnode.props) {
    let expr;
    if ('v-if' in vnode.props) {
      expr = vnode.props['v-if'];
    } else {
      // v-else: invert previous v-if
...
// 3) Patch: update all dynamic text, attributes, and v-if/v-else conditions
